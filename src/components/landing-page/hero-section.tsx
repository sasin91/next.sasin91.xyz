...
export default function HeroSection({ lang }: { lang: Lng }) {
    ...
    logo
        .resize(fill().width(1280).height(1024))
                        ...
                        <div className="flex mt-6 space-x-6 md:order-2">
                            {social.map(item => (
                                <a key={item.name} href={item.href}
                                    ...
                                    <i className={`w-6 h-6 text-cyan-500 group-hover:text-indigo-600 bg-left-bottom bg-gradient-to-r from-indigo-200 via-violet-400 to-cyan-200 bg-[length:0%_2px] bg-no-repeat group-hover:bg-[length:100%_2px] transition-all duration-500 ease-out`} aria-hidden="true" />
                                </a>
                            ))}
                        </div>
                    </div>
                    <AdvancedImage cldImg={logo} plugins={[responsive(), placeholder(), lazyload()]} className="mt-10 aspect-[6/5] w-full max-w-lg rounded-2xl object-cover sm:mt-16 lg:mt-0 lg:max-w-none xl:row-span-2 xl:row-end-2 xl:mt-36" />
                </div>
            </div>
